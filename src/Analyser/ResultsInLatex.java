...
public class ResultsInLatex {
	...
	void PDBTable(Vector<Vector<DataContainer>> Container) throws IOException {
		...
		for(int i=0; i < Container.get(0).size() ; ++i) {
			String PDBID=Container.get(0).get(i).PDB_ID.contains("-") ? Container.get(0).get(i).PDB_ID.substring(0, Container.get(0).get(i).PDB_ID.indexOf('-')) : Container.get(0).get(i).PDB_ID; 
			String Simulated="1";
			if(!PDB.contains(PDBID)) {
				Simulated="2";
			CSV+=PDBID +","+Container.get(0).get(i).Resolution+","+Container.get(0).get(i).F_mapCorrelation+","+Simulated+","+Container.get(0).get(i).E_mapCorrelation+"\n";
	...
	void OverallResults(String ResultsDir) throws IOException {
		...
		Vector<OverallResult> Results = new Vector<OverallResult>();
		 for(int i=0 ;i <Container.size() ; ++i ) {
			 ...
			 CSV+=e.ToolsNames.get(i) +","+Completed+","+Intermediate+","+Failed+","+e.ToolsNames.get(i).substring(e.ToolsNames.get(i).length()-5, e.ToolsNames.get(i).length())+"\n";
			 Results.add(new OverallResult(e.ToolsNames.get(i).substring(0, e.ToolsNames.get(i).length()-5) ,e.ToolsNames.get(i).substring(e.ToolsNames.get(i).length()-5, e.ToolsNames.get(i).length()),Completed,Intermediate,Failed,IntermediateTime ));
		 }
		 ...
		 for(int i=0 ; i < Results.size() ; ++i) {
			 ...
			 for(int dm=0 ; dm < Results.size() ; ++dm) {
				 if(Results.get(i).FileName.equals(Results.get(dm).FileName)){
					 if(Results.get(dm).DM.equals("hancs")) { 
						 hancs= "\\tiny "+ Results.get(dm).Completed +"&" + "\\tiny "+ Results.get(dm).IntermediateTime + "(T) "+Results.get(dm).Intermediate+"(C) &"+"\\tiny "+ Results.get(dm).Failed;
					 }
					 if(Results.get(dm).DM.equals("mrncs")) {
						 mrncs= "\\tiny "+Results.get(dm).Completed +"&" + "\\tiny "+Results.get(dm).IntermediateTime + "(T) "+Results.get(dm).Intermediate+"(C) &"+"\\tiny "+ Results.get(dm).Failed;
					 }
					 if(Results.get(dm).DM.equals("noncs")) {
						 noncs= "\\tiny "+Results.get(dm).Completed +"&" + "\\tiny "+Results.get(dm).IntermediateTime + "(T) "+Results.get(dm).Intermediate+"(C) &"+"\\tiny "+Results.get(dm).Failed;
					 }
				 }
			 }
			...
			//System.out.println(Results.get(i).DM);
			 ...
			 System.out.println("\\scriptsize "+ Results.get(i).FileName+"&&"+hancs+"&&"+mrncs+"&&"+noncs+"\\\\");
	...
	void PDBList(String ResultsDir) throws IOException{
		...
		Vector<Vector<DataContainer>> Container = new Vector<Vector<DataContainer>>();
		 ...
		 for(int i=0 ;i <Container.size() ; ++i ) {
			 ...
			 for(int y=0; y < Container.get(i).size() ; ++y) {
				 ...
				 for (int f=0; f<PDBList.size();++f) {
					 if(PDBList.get(f).PDBIDTXT.equals(Container.get(i).get(y).PDB_ID.substring(0, 4)) && PDBList.get(f).Resolution.equals(Container.get(i).get(y).Resolution))
						 Found=true;
				 }
		 ...
		 for(int i=0 ; i < PDBList.size() ; ++i) {
			 ...
			 for (int f=0; f<CheckedPDB.size();++f) {
				...
				// System.out.println(CheckedPDB.get(f).Resolution);
				 if(CheckedPDB.get(f).PDBIDTXT.equals(PDBList.get(i).PDBIDTXT) && CheckedPDB.get(f).Resolution.equals(PDBList.get(i).Resolution))
					 NotFoundAtAll=true;
		 ...
		 for(int i=0 ; i < CheckedPDB.size() ; ++i) {
			 ...
			 TableContext+="\\tiny "+CheckedPDB.get(i).PDBIDTXT+"&& \\tiny"+ CheckedPDB.get(i).Resolution+"&& \\tiny "+CheckedPDB.get(i).F_mapCorrelation+"& \\tiny "+CheckedPDB.get(i).E_mapCorrelation+"\\\\"+"\n";
		 }
...
//System.out.println(PDBList.get(m).Resolution);
		 ...
		 for (int m = 0; m < Reso.size(); ++m) {
				...
				ResoTable+=" &"+ " \\scriptsize From "+Reso.get(m)+" to less than "+(Reso.get(m)+1)+" & \\scriptsize "+NumReso.get(m)+" \\\\ \n";
			}
		 ...
		 for (File Folder : Folders) {
			...
			System.out.println(Folder.getName());
			 ...
			 for(int i=0 ; i < Container.size() ; ++i) {
				 ...
				 Container2.addElement(e.CheckPDBexists(Container,Container.get(i))); // With intermediate 
			 ...
			 */
			 ...
			 for(int i=0 ; i < Container2.size() ; ++i) {
				...
				for(int B=0; B<Container2.get(i).size();++B ) {
					...
					if(!Container2.get(i).get(B).Completeness.equals("None") && WorstCase.Completeness.equals("None")) {// assign first value and assume is the worst
						WorstCase=Container2.get(i).get(B);
						//WorstCase.Completeness=Container2.get(i).get(B).Completeness;
					}
					if(!Container2.get(i).get(B).Completeness.equals("None") && Integer.valueOf(Container2.get(i).get(B).Completeness) > Integer.valueOf(BestCase.Completeness)) {
						BestCase=Container2.get(i).get(B);
					}
					if(!Container2.get(i).get(B).Completeness.equals("None") && Integer.valueOf(Container2.get(i).get(B).Completeness) < Integer.valueOf(WorstCase.Completeness)) {
						WorstCase=Container2.get(i).get(B);
					}
				}
				}
				...
				for(int Other=0 ; Other < Container2.size() ; ++Other) {
					...
					for(int t=0 ; t < Container2.get(Other).size() ; ++t) {
						...
						if(Container2.get(Other).get(t).PDB_ID.equals(BestCase.PDB_ID) && !Container2.get(Other).get(t).Completeness.equals("None") &&Integer.valueOf(BestCaseCompareTo) <= Integer.valueOf(Container2.get(Other).get(t).Completeness)) {
							//System.out.println(e.ToolsNames.get(Other));
							//System.out.println("The Best Case in other  "+ Container2.get(Other).get(t).PDB_ID +" "+Container2.get(Other).get(t).n1m2);
						...
						if(Container2.get(Other).get(t).PDB_ID.equals(WorstCase.PDB_ID)  &&!Container2.get(Other).get(t).Completeness.equals("None")&& Integer.valueOf(WorstCaseComapreTo) >=  Integer.valueOf(Container2.get(Other).get(t).Completeness) ) {
							//System.out.println(e.ToolsNames.get(Other));
							//System.out.println("The Worst Case in the other  "+ Container2.get(Other).get(t).PDB_ID +" "+Container2.get(Other).get(t).n1m2);
				...
				//TableOfBestCases+=" \\tiny "+e.ToolsNames.get(i)+"(Best Case)" +"& \\tiny "+Reso.get(r)+" to less than "+(Reso.get(r)+1)+" & \\tiny "+e.ToolsNames.get(OtherToolBest)+"&& \\tiny "+BestCase.PDBIDTXT+" & \\tiny "+EMapBest+" & \\tiny "+PDB1Best+" & \\tiny "+BestCaseCompareTo+" & \\tiny "+RfactorBest+" & \\tiny "+TimeTakingBest+" \\\\ \n"; ;
				//TableOfWorstCases+="\\tiny "+ e.ToolsNames.get(i)+"(Worst Case)" +"& \\tiny "+Reso.get(r)+" to less than "+(Reso.get(r)+1)+" & \\tiny "+e.ToolsNames.get(OtherToolWorst)+"&& \\tiny "+WorstCase.PDBIDTXT+" & \\tiny "+EMapWorst+" & \\tiny "+PDB1Worst+" & \\tiny "+WorstCaseComapreTo+" & \\tiny "+RfactorWorst+" & \\tiny "+TimeTakingWorst+" \\\\ \n"; ;

				
				TableOfBestCases+=" \\tiny "+e.ToolsNames.get(i)+"(Best Case)" +"& \\tiny "+Reso.get(r)+" to less than "+(Reso.get(r)+1)+" & \\tiny "+e.ToolsNames.get(OtherToolBest)+"&& \\tiny "+BestCase.PDBIDTXT+" & \\tiny "+EMapBest+" & \\tiny "+PDB1Best+" & \\tiny "+BestCase.Completeness+"\\% "+BestCaseCompareTo+"\\% & \\tiny "+BestCase.R_factor0Cycle+" "+RfactorBest+" & \\tiny "+BestCase.TimeTaking+" "+TimeTakingBest+" \\\\ \n"; ;
				TableOfWorstCases+="\\tiny "+ e.ToolsNames.get(i)+"(Worst Case)" +"& \\tiny "+Reso.get(r)+" to less than "+(Reso.get(r)+1)+" & \\tiny "+e.ToolsNames.get(OtherToolWorst)+"&& \\tiny "+WorstCase.PDBIDTXT+" & \\tiny "+EMapWorst+" & \\tiny "+PDB1Worst+" & \\tiny "+WorstCase.Completeness+"\\% "+WorstCaseComapreTo+"\\% & \\tiny "+WorstCase.R_factor0Cycle+" "+RfactorWorst+" & \\tiny "+WorstCase.TimeTaking+" "+TimeTakingWorst+" \\\\ \n"; ;
		...
		File[] Folders = new File(ResultsDir).listFiles();
		 ...
		 for (File Folder : Folders) {
			 ...
			 if (Folder.isDirectory()) {
				 ...
				 /*
				 // Add the other DM to this Container in order to exclude  the cases were not built 
				 Vector<Vector<DataContainer>> AllDMResults = new Vector<Vector<DataContainer>>();
				 for (File FFolder : Folders) {
			...
			int OrginalContainerSize=Container.size();
			Container.addAll(AllDMResults);
			*/
				 Vector<Vector<DataContainer>> Container2 = new Vector<Vector<DataContainer>>();
			 for(int i=0 ; i < Container.size() ; ++i) {
			 ...
			 */
			 ...
			 System.out.println("Container size "+Container.size());
			 Vector<Integer>Reso =  GroupedResults(Container.get(0)); // because all excels have the same cases so one is engouh 
				for(int r=0; r < Reso.size() ; ++r) {
					...
					for (int y = 0 ; y <Container.get(i).size() ; ++y ) {
						...
						/*
						if(GroupedByPhases==true) {
							 if(NumberUtils.isParsable(Container.get(i).get(y).E_mapCorrelation))
							 Value = Double.parseDouble(Container.get(i).get(y).E_mapCorrelation)*10;
							 else
								 Value =-1.0;// not calculated phases 	 
						}
						*/
						
						if (Reso.get(r).equals(Value.intValue()))  {
							ToBeInExcel.add(Container.get(i).get(y));
							ToBeInExcel.get(ToBeInExcel.size()-1).PDB_ID=e.ToolsNames.get(i);// to save the name of tool instead of PDB ID 
						}
	...
	void SpssBootstraping(String ResultsDir) throws StatsException {
		...
		new RunComparison().CheckDirAndFile("SpssExcelResults");
		 ...
		 for (File Folder : Folders) {
			 ...
			 String Des=  "/DESTINATION FORMAT =XLSX OUTFILE = 'SpssExcelResults/"+Folder.getName()+"/"+Excel.getName()+"'.";
			 StatsUtil.start(); 
	...
	void ReadingSpssBootstraping (String ResultsDir) throws IOException {
		...
		String TheFullTable="";
		 for (File Folder : Folders) {
			 ...
			 Vector<SPSSDescriptivesTable> ThisExcel= new SPSS().DescriptivesTableReader(Excel.getAbsolutePath());
		 System.out.println(Excel.getAbsolutePath());
		...
		//	String ResoRow=" \\footnotesize \\parbox[t]{-10mm}{\\multirow{PipelinesNum}{2.3cm}{\\rotatebox[origin=c]{90}{From R1 \\r{A}  to less than R2 \\r{A}}}}";
			...
			TheFullTable+=ResoRow+" & \\tiny "+table.get(i).Pipeline +"& "+ RowHancs +" "+ RowMrncs +" "+ RowNoncs+" \\\\ \n"; ;
			}
			}
		...
		Vector<Vector<DataContainer>> Container = new Vector<Vector<DataContainer>>();
		 ...
		 for(int i=0; i < Container.size() ; ++i)
			 for(int m=0; m < Container.get(i).size() ;++m) {
				 ...
				 if(NumberUtils.isParsable(Container.get(i).get(m).E_mapCorrelation)&& Double.parseDouble(Container.get(i).get(m).E_mapCorrelation) < min)
					 min = Double.parseDouble(Container.get(i).get(m).E_mapCorrelation);
				 if(NumberUtils.isParsable(Container.get(i).get(m).E_mapCorrelation)&& Double.parseDouble(Container.get(i).get(m).E_mapCorrelation) > max)
...
//System.out.println(PDBList.get(m).Resolution);
		 ...
		 for (int m = 0; m < Reso.size(); ++m) {
				...
				ResoTable+=" &"+ " \\tiny From "+Reso.get(m)+" to less than "+(Reso.get(m)+1)+" & \\tiny "+NumReso.get(m)+" \\\\ \n";
			}
			if(!new File("Latex/ResoTable.tex").exists()) {
			...
			for(int n=0 ; n <ExcelNames.get(i).ToolsNames.size() ; ++n ) {
				if(!TableHeader.contains(ExcelNames.get(i).ToolsNames.get(n))) {
					...
					ExcelNames.get(i).ToolsNames.get(n).equals("Phenix.xlsx") 
						...
						TableHeader+=" \\multicolumn{2}{c}{ \\tiny "+ ExcelNames.get(i).ToolsNames.get(n) +"} &";
		...
		// for(int i=0 ;i < ExcelNamesAsInTableHeader.size() ; ++i) { // looping on pipelines 
			 ...
			 for(int d=0 ; d <DatasetNames.size() ; ++d ) { // looping on datasets 
				...
				// String ZeroPercentRow=" & \\tiny "+DatasetNames.get(d)+" 0\\%";
				 ...
				 for(int c=0 ; c < AllDatasetContainer.get(d).size() ; ++c) {// Finding the index for the containers 
					 ...
					 for(int compareTo=0 ; compareTo < AllDatasetContainer.get(d).get(IndexForTheHeaderContainer).size() ; ++compareTo) {
						...
						if(AllDatasetContainer.get(d).get(IndexForTheHeaderContainer).get(compareTo).Completeness.equals("None"))
							...
							if( Integer.parseInt(AllDatasetContainer.get(d).get(IndexForTheRowContainer).get(c).Completeness) >= Integer.parseInt(AllDatasetContainer.get(d).get(IndexForTheHeaderContainer).get(compareTo).Completeness)) {
								...
								if( (Integer.parseInt(AllDatasetContainer.get(d).get(IndexForTheRowContainer).get(c).Completeness) - Integer.parseInt(AllDatasetContainer.get(d).get(IndexForTheHeaderContainer).get(compareTo).Completeness)) >=5) {
								{
									CountFivePrecentge++;
								}
...
class OverallResult{
	...
	public OverallResult(String filename, String dm , int completed , int intermediate , int failed , int intermediateTime) {
